...
type ThreeDSApiThreeDSCreateSessionRequest struct {
	ctx                         context.Context
	ApiService                  *ThreeDSApiService
...
func (r ThreeDSApiThreeDSCreateSessionRequest) CreateThreeDSSessionRequest(createThreeDSSessionRequest CreateThreeDSSessionRequest) ThreeDSApiThreeDSCreateSessionRequest {
	r.createThreeDSSessionRequest = &createThreeDSSessionRequest
	return r
}

func (r ThreeDSApiThreeDSCreateSessionRequest) Execute() (*CreateThreeDSSessionResponse, *http.Response, error) {
	return r.ApiService.ThreeDSCreateSessionExecute(r)
}
...
ThreeDSCreateSession Method for ThreeDSCreateSession
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ThreeDSApiThreeDSCreateSessionRequest
*/
...
func (a *ThreeDSApiService) ThreeDSCreateSessionExecute(r ThreeDSApiThreeDSCreateSessionRequest) (*CreateThreeDSSessionResponse, *http.Response, error) {
	var (
		...
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *CreateThreeDSSessionResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ThreeDSApiService.ThreeDSCreateSession")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/3ds/sessions"
	...
	// body params
	localVarPostBody = r.createThreeDSSessionRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKey"]; ok {
